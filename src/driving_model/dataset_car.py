...
class DrivingCaptures(Dataset):
   def __init__(self, root_dir, labels, col='speed', is_train=True):
   ...
   def __getitem__(self, idx):
        ...
        frame = float(filename.split('/')[-1].split('.')[0])
        csv_name = folder + ".csv"
        df = pd.read_csv(os.path.join(self.root_dir, csv_name))
        target = df.loc[df['frame'] == int(frame), [self.col]].values
        return img, target
...
if __name__ == "__main__":
     ...
     train_set = DrivingCaptures(train_root_dir, labels, 'speed')
     img, target = train_set[1000]
     print(target)